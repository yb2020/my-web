var businessShortRateArr6 = [5.1, 5.35, 5.6, 5.85, 6.1, 5.85, 5.6, 5.6, 5.35, 5.1, 4.80, 4.6, 4.35],
    businessShortRateArr12 = [5.56, 5.81, 6.06, 6.31, 6.56, 6.31, 6, 5.6, 5.35, 5.1, 4.80, 4.6, 4.35],
    ...
    PAFShortRateArr = [3.5, 3.75, 4, 4.2, 4.45, 4.2, 4, 3.75, 3.5, 3.25, 3, 2.6, 2.6],
    PAFLongRateArr = [4.05,4.3, 4.5, 4.7, 4.9, 4.7, 4.5, 4.25, 4, 3.75, 3.5, 3.1, 3.1],
    loanType = 0,
    loanPeriods = 240,
...
var LPR_1year_value = 3.45;
var LPR_5year_value = 4.20;

$(function() {
    $("#business_LPR").val(LPR_5year_value);
    $("#business_calc").on("click", function() {
        0 <= $(this).attr("class").indexOf("normal-tab") && ($(this).removeClass("normal-tab").addClass("select-tab").siblings(".tab").removeClass("select-tab").addClass("normal-tab"), $("#business_sum_line").show(), $("#business_rate_select_line").show(), $("#business_rate_value_line").show(), $("#PAF_sum_line").hide(), $("#PAF_rate_line").hide(), $("#business_interest_total_debx").hide(), $("#PAF_interest_total_debx").hide(), $("#business_interest_total_debj").hide(),
            $("#PAF_interest_total_debj").hide(), $("#business_atc_recmd_mod").show(), $("#PAF_atc_recmd_mod").hide(),$("input[name='repayTypeS']").change(),  loanType = 0)
    });
    $("#PAF_calc").on("click", function() {
        0 <= $(this).attr("class").indexOf("normal-tab") && ($(this).removeClass("normal-tab").addClass("select-tab").siblings(".tab").removeClass("select-tab").addClass("normal-tab"), $("#PAF_sum_line").show(), $("#PAF_rate_line").show(), $("#business_sum_line").hide(), $("#business_rate_select_line").hide(), $("#business_rate_value_line").hide(), $("#business_interest_total_debx").hide(),
            $("#PAF_interest_total_debx").hide(), $("#business_interest_total_debj").hide(), $("#PAF_interest_total_debj").hide(), $("#PAF_atc_recmd_mod").show(), $("#business_atc_recmd_mod").hide(),  $("#business_rate_select_2").hide(),loanType = 1)
    });
    $("#mix_calc").on("click", function() {
        0 <= $(this).attr("class").indexOf("normal-tab") && ($(this).removeClass("normal-tab").addClass("select-tab").siblings(".tab").removeClass("select-tab").addClass("normal-tab"), $("#business_sum_line").show(), $("#business_rate_select_line").show(), $("#business_rate_value_line").show(),
            $("#PAF_sum_line").show(), $("#PAF_rate_line").show(), $("#business_interest_total_debx").show(), $("#PAF_interest_total_debx").show(), $("#business_interest_total_debj").show(), $("#PAF_interest_total_debj").show(), $("#business_atc_recmd_mod").show(), $("#PAF_atc_recmd_mod").hide(),$("input[name='repayTypeS']").change(), loanType = 2)
    });
    ...
    // 新加业务逻辑
    $("#business_BP").width($("#business_rate_select_2").width()*0.33-36);
    $("#business_LPR").width($("#business_rate_select_2").width()*0.33-36);
    $("input[name='repayTypeS']").change(function(){
        if($('input[name=repayTypeS]:checked').val()==1){
            $("#business_rate_select_1").show();
            $("#business_rate_select_2").hide();
            businessRateUpdate();
        }else{
            $("#business_rate_select_1").hide();
            $("#business_rate_select_2").show();
            $("#business_BP").width($("#business_rate_select_2").width()*0.33-36);
            $("#business_LPR").width($("#business_rate_select_2").width()*0.33-36);
            businessRateUpdateNew();
        }
    });

    $("#business_LPR").on("change", function() {
        businessRateUpdateNew();
    });
    $("#business_BP").on("change", function() {
        businessRateUpdateNew();
    });
    $("#business_LPR").on("blur", function() {
        businessRateUpdateNew();
    });
    $("#business_BP").on("blur", function() {
        businessRateUpdateNew();
    });
    $("#business_rate").on("change", function() {
        update_LPR_BP();
    });
    $("#business_rate").on("blur", function() {
        update_LPR_BP();
    });


    function businessRateUpdateNew() {
        var a = 0;
        var business_LPR_value = parseFloat($("#business_LPR").val());
        var business_BP_value = parseFloat($("#business_BP").val())||0;
        a = Math.round(100 * business_LPR_value+business_BP_value) / 100;
        $("#business_rate").val("" + a);
    }
    function update_LPR_BP() {
        var a = 0;
        var b = 0;
        var business_LPR_value = parseFloat($("#business_LPR").val());
        var business_BP_value = parseFloat($("#business_BP").val())||0;
        a = Math.round(100 * business_LPR_value+business_BP_value) / 100;
        b = parseFloat($("#business_rate").val());
        if(a!=b){
            $("#business_LPR").val("");
            $("#business_BP").val("");
        }
...
function businessRateUpdate() {
    // 新加业务逻辑
    var repayTypeS = $('input[name="repayTypeS"]:checked').val();
    if(repayTypeS==1){
        var a = 0;
        ...
        var rate = loanPeriods>=60?LPR_5year_value:LPR_1year_value;
        $('#business_LPR').val(rate);
        var business_BP_value = +parseFloat($("#business_BP").val())||0;
        $("#business_rate").val(rate+business_BP_value/100);
    }
}
...
function businessRateInputCheck() {
    if(loanType==0){
        var rate = $("#business_rate").val();
        var lpr = +$('#business_LPR').val();
        var bp = +$('#business_BP').val()||0;
        if(!rate){
            $("#business_rate").val(+(lpr+bp/100).toFixed(6));
        }
    }
...
function calculate_debx_singleLoan(a, b) {
    ...
    $("#loan_total_1").text("" + a +"元");
    $("#period_total_1").text("" + (loanPeriods/12) + "年("+loanPeriods+"期)");
    f = d / loanPeriods;
...
function calculate_debj_singleLoan(a, b) {
    ...
    $("#loan_total_2").text("" + a +"元");
    $("#period_total_2").text("" + (loanPeriods/12) + "年("+loanPeriods+"期)");
    g = a / loanPeriods;
    g = Math.round(100 * g) / 100;
    var m = 0,
        h = 0,
