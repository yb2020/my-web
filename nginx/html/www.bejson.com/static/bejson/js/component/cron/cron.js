...
function view_result(){
    if($('#cron').val()){
        ...
        $api_bejson = '//api.bejson.com/';
        doPost($api_bejson + 'btools/othertools/cron/', {"crontxt": $('#cron').val()}, function (data) {
            if(data.code === 0 || data.code === '0') {
                $('#jsonlintResult').html(data.obj);
                $('#jsonlintBox').addClass('alert').removeClass('alert-warning alert-danger');
            } else {
                alertError({con: data.message})
            }
            $('body').toggleClass('loading');
        }, function () {
