...
var templateHelpers = {
	loadedImage: function(canvas, file) {
		...
		canvas.fileName = "_"+replaceNonAlphabetCharsWithHex(file.extra.nameNoExtension);
...
function replaceNonAlphabetCharsWithHex(str) {
	// 使用正则表达式匹配非a-z的字符，并将其替换为十六进制格式
	return str.replace(/[^_a-zA-Z0-9-]/ig, function(char) {
		// 将字符转换为十六进制格式
		const hexCode = char.charCodeAt(0).toString(16);
		// 返回十六进制格式的字符
		return 'x' + hexCode; // 使用\\x前缀表示十六进制字符
	});
}
...
sprite.getopts = function() {
	...
	return {
		...
		saomiao: (direction == "saomiao"),
	}
};

sprite.getcanvases = function() {
	var e= elements.addedFiles.children().map(function(i, el) {
		return $(this).find("canvas")[0];
	});
	//console.log(e);

	e.sort((a, b) => {
		const heightA = a.originalCanvas.height; // 获取Canvas元素的高度
		const heightB = b.originalCanvas.height; // 获取Canvas元素的高度
		return heightA-heightB; // 根据高度升序排序，如果需要降序排序，可以使用 heightB - heightA
	});

	// $.each(e,function (i,n){
	// 	console.log(i+":"+n.originalCanvas.height)
	// })
...
sprite.mergefiles = function() {
	...
	var MAX_WIDTH=parseInt(document.getElementById('zh_max_width').value);
	var ROW_HEIGHT=0;
	...
	sprite.eachcanvas(function(canvas, i, islast) {
		...
		if (isDiagonal) {
			...
			if((CU_X+canvas.width)>MAX_WIDTH){
				CU_X=0;
				CU_Y+=ROW_HEIGHT+opts.spacing;
				ROW_HEIGHT=0;

				canvas.storeX = CU_X;
				canvas.storeY = CU_Y;

				//console.log("换行了X"+CU_X+"Y"+CU_Y);

				CU_X+=canvas.width+opts.spacing;
			}
			else
			{
				canvas.storeX = CU_X;
			...
			if(canvas.height>ROW_HEIGHT)
			{
				ROW_HEIGHT=canvas.height;
			}

			//console.log("最大高度"+ROW_HEIGHT);
